...
def _random_inputs(
    ...
    *,
    ...
    bz_grid_addresses = rng.integers(
        -2, 3, size=(num_grid, 3), dtype="int64"
    )
    ...
    for i in range(num_triplets):
        if rng.random() < u_fraction:
            ...
            matches = np.where(
                (bz_grid_addresses == inv[None, :]).all(axis=1)
            )[0]
            if matches.size == 0:
                triplets[i] = rng.integers(1, num_grid, size=3, dtype="int64")
            else:
    ...
    g = rng.standard_normal(
        (2, num_triplets, num_band0, num_band, num_band)
    ).astype("double")
    g_zero = (
...
def _run_c(
    ...
    detailed = np.zeros(
        (num_triplets, num_band0, num_band, num_band), dtype="double"
    )
