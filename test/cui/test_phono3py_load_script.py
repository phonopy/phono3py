...
def test_phono3py_load_exclude_gamma_acoustic():
    """Test phono3py-load script with --exclude-gamma-acoustic by Si.

    The frequencies of the acoustic modes at Gamma written in kappa-m555.hdf5
    are exactly zero.
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            with h5py.File("kappa-m555.hdf5", "r") as f:
                np.testing.assert_array_equal(f["frequency"][0, :3], 0)  # type: ignore
