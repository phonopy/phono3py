    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            with h5py.File(kappa_path, "r") as f:
                ...
                assert f["mode_kappa_P_RTA"].shape[-1] == 6  # Voigt
                # mode_kappa_C: (num_temp, num_gp, num_band0, num_band, 6)
                assert f["mode_kappa_C"].ndim == 5
                assert f["mode_kappa_C"].shape[0] == 1  # num_temp
                assert f["mode_kappa_C"].shape[-1] == 6  # Voigt
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            with h5py.File("kappa-m999.hdf5", "r") as f:
                ...
                assert f["mode_kappa_P_exact"].shape[0] == 1
                assert f["mode_kappa_P_exact"].shape[-1] == 6
                assert f["mode_kappa_P_RTA"].ndim == 4
                assert f["mode_kappa_P_RTA"].shape[0] == 1
                assert f["mode_kappa_P_RTA"].shape[-1] == 6
                # mode_kappa_C: (num_temp, num_gp, num_band0, num_band, 6)
                assert f["mode_kappa_C"].ndim == 5
                assert f["mode_kappa_C"].shape[0] == 1
                assert f["mode_kappa_C"].shape[-1] == 6
                assert f["temperature"][0] == pytest.approx(300.0)
