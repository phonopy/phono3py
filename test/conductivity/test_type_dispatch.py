...
def test_get_rta_writer_grid_data_from_attr_capabilities():
    ...
    data = get_rta_writer_grid_data(br_like, 0)
    np.testing.assert_allclose(data["group_velocities_i"], br_like.group_velocities[0])
    np.testing.assert_allclose(data["gv_by_gv_i"], br_like.gv_by_gv[0])
    np.testing.assert_allclose(data["velocity_operator_i"], velocity_operator[0])
    np.testing.assert_allclose(
        data["mode_heat_capacities"], br_like.mode_heat_capacities
    )
...
def test_get_rta_writer_kappa_data_from_attr_capabilities():
    ...
    assert data["kappa"] is br_like.kappa
    assert data["mode_kappa"] is br_like.mode_kappa
    assert data["group_velocities"] is br_like.group_velocities
    assert data["gv_by_gv"] is br_like.gv_by_gv
    assert data["kappa_TOT_RTA"] is br_like.kappa_TOT_RTA
    assert data["kappa_P_RTA"] is br_like.kappa_P_RTA
    assert data["kappa_C"] is br_like.kappa_C
    assert data["mode_kappa_P_RTA"] is br_like.mode_kappa_P_RTA
    assert data["mode_kappa_C"] is br_like.mode_kappa_C
    assert data["mode_heat_capacities"] is br_like.mode_heat_capacities
...
def test_get_lbte_writer_kappa_data_from_attr_capabilities():
    ...
    assert data["kappa"] is lbte_like.kappa
    assert data["mode_kappa"] is lbte_like.mode_kappa
    assert data["kappa_RTA"] is lbte_like.kappa_RTA
    assert data["mode_kappa_RTA"] is lbte_like.mode_kappa_RTA
    assert data["group_velocities"] is lbte_like.group_velocities
    assert data["gv_by_gv"] is lbte_like.gv_by_gv
    assert data["kappa_P_exact"] is lbte_like.kappa_P_exact
    assert data["kappa_P_RTA"] is lbte_like.kappa_P_RTA
    assert data["kappa_C"] is lbte_like.kappa_C
    assert data["mode_kappa_P_exact"] is lbte_like.mode_kappa_P_exact
    assert data["mode_kappa_P_RTA"] is lbte_like.mode_kappa_P_RTA
    assert data["mode_kappa_C"] is lbte_like.mode_kappa_C
